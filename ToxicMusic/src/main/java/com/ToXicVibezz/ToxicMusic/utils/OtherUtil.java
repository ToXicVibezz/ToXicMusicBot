 ...
 */
package com.ToXicVibezz.ToxicMusic.utils;

import com.ToXicVibezz.ToxicMusic.toxicmusic;
...
public class OtherUtil
{
    public final static String NEW_VERSION_AVAILABLE = "There is a new version of Zero-Two available!\n"
                    + "Current version: %s\n"
                    + "New Version: %s\n\n"
                    + "Please visit https://github.com/ToXicVibezz/ToxicMusic/releases/latest to get the latest release.";
    ...
    public static Path getPath(String path)
    {
        ...
        if(result.toAbsolutePath().toString().toLowerCase().startsWith(WINDOWS_INVALID_PATH))
        {
            try
            {
                result = Paths.get(new File(toxicmusic.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getPath() + File.separator + path);
            }
    ...
    {
        if(toxicmusic.class.getPackage()!=null && toxicmusic.class.getPackage().getImplementationVersion()!=null)
            return toxicmusic.class.getPackage().getImplementationVersion();
        else
            return "UNKNOWN";
    }
    ...
    public static String getLatestVersion()
    {
        try
        {
            Response response = new OkHttpClient.Builder().build()
                    .newCall(new Request.Builder().get().url("https://api.github.com/repos/ToXicVibezz/ToxicMusic/releases/latest").build())
                    .execute();
